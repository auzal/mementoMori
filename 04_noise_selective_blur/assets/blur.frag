...
float rangeConvert(float value,float start1, float stop1, float start2, float stop2){
...
void main() {
	...
	if(value < threshold + tolerance && value > threshold - tolerance ){
		...
		if(value < threshold){
			mult = rangeConvert(value, threshold - tolerance, threshold, 0.0, 1.0);
			}else{
				mult = rangeConvert(value, threshold, threshold + tolerance, 1.0, 0.0);
			}
		}

		float sizeX = texelSize.x *mult;
		float sizeY = texelSize.y *mult;

		vec2 size = vec2(sizeX, sizeY);

		// use our blur function
		vec3 blur = gaussianBlur(tex0, uv, size * direction);
		//	if(value > threshold){
